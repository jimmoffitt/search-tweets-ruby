...
class Relational
	...
	#Currently, this class is seeded with connection metadata when the object is created. This class could easily learn
	#to open and parse the config file, but currently does not.
	def initialize(host=nil, port=nil, database=nil, credentials=nil)
		@connected = false
	...
	def connect
		begin
			@db_engine = Mysql2::Client.new(:host => @host, :port => @port, :username => @credentials['username'], :password => @credentials['password'], :database => @database )
	...
	#Although with Search, there is always just one rule, would like to keep this code general enough to work with other
	#APIs, like HPT and real-time that support multiple rules.
	def write_matching_rules(rules)
		begin
			rule_values = []
				
			rules.each do |rule_metadata|
				tweet_id = rule_metadata['tweet_id']
				
				rule_metadata['rules'].each do |item|
					values = "(#{tweet_id}, '#{ @db_engine.escape(item['filter'])}' , '#{ @db_engine.escape(item['tag'])}', '#{item['rule_id']}', UTC_TIMESTAMP(), UTC_TIMESTAMP())"
					rule_values << values
				end
			end
				
			rules_number = rule_values.length
				
			rule_values = rule_values.join(',')

			#Build query pattern
			sql = "REPLACE INTO matching_rules (tweet_id, filter, tag, rule_id, created_at, updated_at)" +
				" VALUES #{rule_values};"

			result = @db_engine.query(sql)
			puts "Stored metadata for #{rules_number} matching rules..."
					
		rescue Exception => e
			puts "Error storing #{rules_number} matching rules..."
			puts e.message
			puts e.backtrace.inspect
		end
	...
	def transform_link_objects(links)
		begin
			links.each do |link|
				...
				link['unwound']['title'] = @db_engine.escape(link['unwound']['title']) if !link['unwound']['title'].nil?
				...
				link['unwound']['description'] = @db_engine.escape(link['unwound']['description']) if !link['unwound']['description'].nil?
			end
		rescue Exception => e
			puts e.message
			puts e.backtrace.inspect
	...
	def transform_user_object(user)
		...
		begin #User attributes.
			...
			user['name']	= user['name'].gsub("’", "")
			user['name'] = @db_engine.escape(user['name'])

			user['klout_score'] = 'null' if user['klout_score'].nil?
			user['utc_offset'] = 'null' if user['utc_offset'].nil?
			user['time_zone'] =  @db_engine.escape(user['time_zone']) if !user['time_zone'].nil?
			...
			if user['description'].nil?
				...
				user['bio'] = @db_engine.escape(user['bio'])
			end
	...
	def transform_tweet_object(tweet)
		...
		begin
			...
			tweet['source'] = @db_engine.escape(tweet['source'])
	...
	def store_tweets(results)
		...
		results.each do |tweet|
			...
			if !tweet['matching_rules'].nil?
				rule_metadata = {}
				tweet['matching_rules'][0]['id'] = 'null'
				rule_metadata['tweet_id'] = tweet['id']
				rule_metadata['rules'] = tweet['matching_rules']
				rules_array << rule_metadata

			end

		end
		
		write_tweets(tweets_array)
		write_users(users_array)
		write_links(links_array) if links_array.length > 0
		write_hashtags(hashtags_array) if hashtags_array.length > 0
		write_mentions(mentions_array) if mentions_array.length > 0
		write_symbols(symbols_array) if symbols_array.length > 0
		write_native_media(media_array) if media_array.length > 0
		write_matching_rules(rules_array) if rules_array.length > 0
